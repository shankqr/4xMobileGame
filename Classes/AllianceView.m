...
@property (nonatomic, strong) NSMutableArray *array_search_object;

@property (nonatomic, strong) NSMutableArray *search_rows;
@property (nonatomic, strong) NSMutableArray *browse_rows;
...
- (void)viewDidLoad
{
    ...
    self.searchBar1.placeholder = NSLocalizedString(@"Search Alliance..", nil);
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    {
        if (self.array_search_object == nil)
        {
            self.array_search_object = [[NSMutableArray alloc] init];
        }
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar
{
    ...
    [self.array_search_object removeAllObjects];
    
    self.ui_cells_array = self.browse_rows;
...
- (void)updateView
{
    NSDictionary *row101 = @{@"h1": NSLocalizedString(@"Active Alliances (Tap for more info)", nil)};
    NSMutableArray *rows1 = [@[row101] mutableCopy];
    
    NSString *service_name = @"GetAllianceTopActive";
    NSString *wsurl = @"";
    ...
    [Globals.i getServerLoading:service_name :wsurl :^(BOOL success, NSData *data)
     {
         if (success)
         {
             ...
             self.browse_rows = [@[rows1] mutableCopy];
...
- (void)searchView:(NSString *)searchText
{
    NSDictionary *row101 = @{@"h1": NSLocalizedString(@"Search Results", nil)};
    NSMutableArray *rows1 = [@[row101] mutableCopy];
    ...
    [Globals.i getServerLoading:service_name :wsurl :^(BOOL success, NSData *data)
     {
         if (success)
         {
             ...
             if (returnArray.count > 0)
             {
                 ...
                 {
                     AllianceObject *aAlliance = [[AllianceObject alloc] initWithDictionary:row1];
                     [self.array_search_object addObject:aAlliance];
             ...
             self.search_rows = [@[rows1] mutableCopy];
             
             self.ui_cells_array = self.search_rows;
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((indexPath.row > 0) && (indexPath.row % 2 == 1) && ([self.array_search_object count] > 0))
    {
        [userInfo setObject:self.array_search_object[index] forKey:@"alliance_object"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ShowAllianceProfile"
                                                            object:self
                                                          userInfo:userInfo];
    }
    else if ((indexPath.row > 0) && (indexPath.row % 2 == 1) && ([self.array_alliance_object count] > 0))
    {
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    UIButton *button1 = [UIManager.i dynamicButtonWithTitle:NSLocalizedString(@"Create Alliance", nil)
                                                     target:self
...
- (void)button1_tap:(id)sender
{
    [[NSNotificationCenter defaultCenter]
     postNotificationName:@"ShowAllianceCreate"
     object:self];
}
