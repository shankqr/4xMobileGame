...
- (void)notificationReceived:(NSNotification *)notification
{
    ...
    else if ([[notification name] isEqualToString:@"ClickCloseTemplateButton"])
    {
        //NSLog(@"ClickCloseTemplateButton title: %@",self.title);
        NSDictionary *userInfo = notification.userInfo;
        if (userInfo != nil)
        {
            NSString* templateTitleToClose = [userInfo objectForKey:@"template_title"];
            if([self.title isEqualToString:templateTitleToClose])
            {
...
- (void)updateView
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(notificationReceived:)
                                                 name:@"ClickCloseTemplateButton"
                                               object:nil];
    
    
    if (self.ivBkgHeader == nil)
    {
        self.ivBkgHeader = [[UIImageView alloc] initWithImage:[UIManager.i imageNamedCustom:@"bkg_header"]];
    ...
    [self.backgroundImage setImage:[UIManager.i imageNamedCustom:@"skin_default"]];
    ...
    if (self.titleLabel == nil)
    {
        ...
        [self.titleLabel setFont:[UIFont fontWithName:DEFAULT_FONT size:DEFAULT_FONT_SIZE]];
    ...
    if (self.closeButton == nil)
    {
        ...
        UIImage *btn_image = [UIManager.i imageNamedCustom:@"button_close1"];
        ...
        [self.closeButton setBackgroundImage:[UIManager.i imageNamedCustom:@"button_close1_hvr"] forState:UIControlStateHighlighted];
        [self.closeButton addTarget:self action:@selector(closeButton_tap:) forControlEvents:UIControlEventTouchUpInside];
        
        [self.view addSubview:self.closeButton];
    }

    if (self.frameType == FRAME_FullScreen) //No borders, fullscreen
    {
        rect.origin.x = 0;
        rect.origin.y = 0;
    ...
    else if (self.frameType == FRAME_BorderTop) //1 border at top
    {
    ...
    else if (self.frameType == FRAME_BorderTopFullBackgroundImage) //1 border spacing at top but no header image
    {
        CGFloat extra_spacing_y = 25.0*SCALE_IPAD;
        
        rect.origin.x = SCREEN_OFFSET_X;
        rect.origin.y = SCREEN_OFFSET_MAINHEADER_Y + extra_spacing_y;
        rect.size.width = self.view.bounds.size.width;
        rect.size.height = UIScreen.mainScreen.bounds.size.height - rect.origin.y;
        
        [self.ivBkgHeader setImage:nil];
    }
    else if (self.frameType == FRAME_FullScreenNoCloseButton) //Fullscreen and no close button
    {
        rect.origin.x = 0;
        rect.origin.y = 0;
        rect.size.width = self.view.bounds.size.width;
    ...
    else if (self.frameType == FRAME_FullScreenChart) //Fullscreen chart dialog box
    {
        ...
        UIImage *bkg_image = [UIManager.i dynamicImage:full_frame prefix:@"bkg6"];
        [self.backgroundImage setImage:bkg_image];
        
        [self.ivBkgHeader setImage:nil];
        
        UIImage *btn_image = [UIManager.i imageNamedCustom:@"button_close2"];
        CGSize btnSize = CGSizeMake(btn_image.size.width * SCALE_IPAD, btn_image.size.height * SCALE_IPAD);
        ...
        [self.closeButton setBackgroundImage:btn_image forState:UIControlStateNormal];
        [self.closeButton setBackgroundImage:[UIManager.i imageNamedCustom:@"button_close2_hvr"] forState:UIControlStateHighlighted];
    }
    else if (self.frameType == FRAME_Plugin) //MainView as background (Plugin system)
    {
    ...
    else if (self.frameType == FRAME_PluginOverlapHeader) //MainView as background (overlap header above)
    {
        rect.origin.x = SCREEN_OFFSET_X;
        rect.origin.y = 0.0f;
        rect.size.width = self.view.bounds.size.width;
    ...
    else if (self.frameType == FRAME_TableFit || self.frameType == FRAME_TableFitNoCloseButton || self.frameType == FRAME_TableFitNotAlwaysOntop || self.frameType == FRAME_TableFitMapDialog)
    {
        CGFloat height = 0;
        CGFloat width = 0;
        ...
        UIImage *bkg_image = [UIManager.i dynamicImage:rect prefix:@"bkg6"];
        [self.backgroundImage setImage:bkg_image];
        [self.backgroundImage setFrame:rect];
    ...
    if (self.frameType == FRAME_TableFit || self.frameType == FRAME_TableFitNoCloseButton || self.frameType == FRAME_TableFitNotAlwaysOntop || self.frameType == FRAME_TableFitMapDialog) //Table fit to screen dialog box style
    {
    ...
    else if (self.frameType == FRAME_FullScreenChart) //Chart dialog box style
    {
        CGFloat dialog_width = UIScreen.mainScreen.bounds.size.width-CHART_CONTENT_MARGIN*3;
        rect.origin.x = (UIScreen.mainScreen.bounds.size.width-dialog_width)/2.0f;
    }
    
    if (self.frameType == FRAME_TableFitNoCloseButton)
    {
        self.closeButton.hidden = YES;
    }
    
    if (self.frameType == FRAME_TableFitMapDialog) //Dialog box ontop of map
    {
        CGFloat spacing = 10.0f*SCALE_IPAD;
...
{
	UIImage *image_h = [UIManager.i dynamicImage:button.frame prefix:@"tab1_hvr"];
...
{
	UIImage *image_h = [UIManager.i dynamicImage:button.frame prefix:@"tab1_hvr"];
    UIImage *image_n = [UIManager.i dynamicImage:button.frame prefix:@"tab1"];
